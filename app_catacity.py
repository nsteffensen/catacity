...
def gconnect():
    ...
    try:
        ...
        # exchange auth code for credentials object
    ...
    if result['issued_to'] != CLIENT_ID:
        response = make_response(
            json.dumps("Token ClientID does not match this app"), 401)
...
def viewItem(category_id, item_id):
    ...
    if 'username' not in login_session \
            or item.acct_id != login_session['user_id']:
        return render_template(
            'publicViewItem.html', category=category, item=item)
    else:
...
def showMyItems():
    items = session.query(Item).filter_by(
      acct_id=login_session['user_id']).all()
...
def editItem(category_id, item_id):
    selItem = session.query(Item).filter_by(
        id=item_id, category_id=category_id).one()

    if 'username' not in login_session \
            or selItem.acct_id != login_session['user_id']:
        return redirect('/login')
    ...
    if request.method == 'POST':
        ...
        return render_template(
            ...
            category=category, item=selItem)
...
def deleteItem(category_id, item_id):
    ...
    if 'username' not in login_session \
            or deleteItem.acct_id != login_session['user_id']:
        return redirect('/login')
    ...
    if request.method == 'POST':
        ...
        category = session.query(Category).filter_by(id=category_id).one()
        return render_template(
          'deleteItem.html', category=category, item=deleteItem)
